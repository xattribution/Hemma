# Sett apps — .exe, .apk, Linux packages
...
the same Sett PWA the browser gets. All the real UI ships from your server —
...
| Artifact | For |
| --- | --- |
| `Sett_x.y.z_x64-setup.exe` / `.msi` | Windows |
| `Sett_x.y.z_amd64.deb` / `.AppImage` | Linux |
| `Sett_x.y.z_*.dmg` | macOS |
| `sett-android.apk` (or the versioned `sett-x.y.z-android.apk`) | Android (sideload) |
...
- **Linux**: `sudo apt install ./Sett_*.deb`, or `chmod +x` the AppImage and
  run it.
...
- with a domain: `https://sett.yourfamily.com`
...
- Plain-`http` LAN addresses work in all shells (the Android build explicitly
  ...
  HTTPS — that's a browser/OS rule, not a Sett one.
