# Coord apps — .exe, .apk, Linux packages
...
the same Coord PWA the browser gets. All the real UI ships from your server —
...
| Artifact | For |
| --- | --- |
| `Coord_x.y.z_x64-setup.exe` / `.msi` | Windows |
| `Coord_x.y.z_amd64.deb` / `.AppImage` | Linux |
| `Coord_x.y.z_*.dmg` | macOS |
...
- **Linux**: `sudo apt install ./Coord_*.deb`, or `chmod +x` the AppImage and
  run it.
- **Android**: open the `.apk` on the phone (send it via the share sheet, a
  link, or a USB copy) and allow "install from unknown sources" when asked.
...
- with a domain: `https://coord.yourfamily.com`
...
- Plain-`http` LAN addresses work in all shells (the Android build explicitly
  ...
  HTTPS — that's a browser/OS rule, not a Coord one.
